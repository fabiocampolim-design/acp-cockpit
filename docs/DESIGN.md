...
| **Security model: 127.0.0.1 only, a random bearer token per launch (cookie, constant-time compare), foreign Host/Origin refused, strict CSP, no external resources.** Optional `--token-file` keeps the token across launches for a stable URL. | Localhost is not a trust boundary; other local processes and DNS-rebinding pages must not reach the session. | No multi-user by design. The server binds `127.0.0.1` and refuses any `Host` that is not loopback (`tests/test_security.py`), so it cannot be reached from another machine directly and cannot be put behind a reverse proxy or a tailnet address as it stands: remote use means forwarding loopback to loopback (SSH `-L`), which leaves every guarantee here intact and is the reader's own risk to take. The persistent token file must be treated like a password. |
...
- **What the adapter drops.** `claude-agent-acp` 0.73 explicitly discards
  ...
  prompt-suggestion one re-verified against the installed 0.79.0 on
  2026-09-20), the
...
- **Collaboration — spec written, implementation pinned (2026-09-06).**
  ...
  it is built on. Only the loopback-to-loopback half of Phase 0 works
  today (an SSH forward plus the token URL, for a trusted group): the
  server binds `127.0.0.1` with no `--bind`, and both the REST and the
  WebSocket guard refuse a foreign `Host`, so a tailnet address or a
  reverse proxy is refused until the work below is done. Everything
  else waits for the author.
