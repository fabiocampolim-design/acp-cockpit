...
| `prompt_suggestion` | `text` | The agent's guess at your next prompt, offered after a turn. Offer it, never send it: the reader accepts or ignores it (the bundled View: a chip over the composer that fills the box). Most adapters do not forward these — claude-agent-acp 0.73 through 0.79.0 discards the SDK message — so a View must work perfectly without ever seeing one. |
...
| `auth_status` | the adapter's `authStatus` passed through: `kind` (`account`), `label` (e.g. `Claude Pro`), `account` (`plan`, `email`, `organization`) — a vendor notification (`_auth/status_update`, claude-agent-acp 0.75.1) the registry lists under `vendor_from_agent_notifications` | Account-wide, not session state: show the label somewhere permanent with the details in a tooltip (the bundled View: a chip at the top right). It carries the user's e-mail, so it never goes into a shared screenshot by default — the tooltip, not the chip. |
| `vendor_update` | `kind` (the `sessionUpdate` value), `update` (the whole update object) | A `session/update` kind outside the ACP schema that this agent's adapter is KNOWN to send (`vendor_update_kinds` in the registry — claude-agent-acp's `subagent_spawned`, `subagent_state_update`, `async_task_*`, and since 0.79.0 `compaction_update` and `compaction_summary_chunk`, the context-compaction lifecycle and the summary it streams). Known is not drift, so no chip; render it as a collapsible row naming the kind, with raw access. `subagent_*` belongs to the `subagents` lane, the rest to `events`. |
