...
- Working codename **CLAUDIU** — internal names may use it; public name decided later (the publication playbook's Phase 3).
- License **Apache-2.0**: `LICENSE` (verbatim), `NOTICE`, and an `SPDX-License-Identifier: Apache-2.0` + copyright header in every `.py` and every non-vendored `.js` file (publication-playbook rule 17).
...
**Interfaces:**
- Produces: `scan_recent_sessions(claude_dir=None, limit_per_project=5, max_lines=200) -> tuple[list, dict]`. First element: `[{"path": str, "sessions": [{"id", "cwd", "summary", "mtime"}]}]` sorted newest-first; second: report dict with keys `project_dirs`, `session_files`, `unreadable_files`, `bad_lines`, `record_types` (dict type→count) — counted, never dropped (publication-playbook rule 14).
...
- Produces: `build_parser() -> argparse.ArgumentParser` (flags: `--config`, `--port`, `--log-dir`, `--no-browser`, `--verbose`, `--quiet`, `--version`; exposed for the docs guard, publication-playbook rule 15); `setup_logging(log_dir, verbose, quiet) -> Path` (returns the audit-log file path); `main(argv=None)`.
...
**Interfaces:**
- Consumes: the whole app via `python -m claudiu`; skips cleanly when Playwright is absent (publication-playbook rule 5).
...
- Produces: docs whose completeness is enforced by tests (publication-playbook rules 10, 13, 15, 17).
